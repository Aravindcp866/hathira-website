...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card w-full max-w-7xl mx-auto relative h-80 my-8 p-12 rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const handleScroll = useCallback(() => {
    ...
    // Debounce updates to reduce jerkiness
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // For window scroll, use the global Lenis instance
      if (globalLenis) {
        console.log('Setting up global Lenis scroll handler');
        globalLenis.on('scroll', handleScroll);
        lenisRef.current = globalLenis;
        return globalLenis;
      } else {
        console.log('Global Lenis not available yet, retrying...');
        return null;
      }
    } else {
      const scroller = scrollerRef.current;
  ...
  useLayoutEffect(() => {
    ...
    // Retry setup if global Lenis isn't ready
    const setupWithRetry = () => {
      const result = setupLenis();
      if (!result && useWindowScroll) {
    ...
    // Fallback: if global Lenis doesn't work, use native scroll
    ...
    return () => {
      ...
      if (lenisRef.current) {
        if (useWindowScroll && globalLenis) {
          // Don't destroy global Lenis, just remove the event listener
